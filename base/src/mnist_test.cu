...
int main() {
	...
	vector<LayerSpecifier> layer_specifier;
	{
		...
		layer0.initializeValues(1, 3, 3, 3, 28, 28, 1, 1, 1, 1);
		LayerSpecifier temp;
	...
	}
	{
		...
		layer0_actv.initializeValues(RELU, 3, 28, 28);
		LayerSpecifier temp;
	...
	{
		FCDescriptor layer1;
		layer1.initializeValues(3 * 28 * 28, 50);
		LayerSpecifier temp;
	...
	}
	{
		...
		layer1_actv.initializeValues(RELU, 50, 1, 1);
		LayerSpecifier temp;
		temp.initPointer(ACTV);
	...
	NeuralNet net(layer_specifier, DATA_FLOAT, batch_size, TENSOR_NCHW, dropout_seed, softmax_eps, init_std_dev, CONV_ALGO_MEMORY_OPTIMAL);
